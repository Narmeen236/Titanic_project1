...
### 2.5 Detailed Explanation on Test Derivations (Requirement 9)

* **All Combinations Coverage (ACoC):**
  * Evaluates combinations of full Cartesian products ($\vert{}P_1\vert{} \times \vert{}P_2\vert{} \times \vert{}P_3\vert{} = 3 \times 3 \times 2 = 18$ combinations). TC01 tests a direct integer lookup with primary attribute assertion $(b_1, b_4, b_7)$, while TC02 exercises a single-word string query evaluating nested collection structures $(b_2, b_4, b_8)$.
* **Each Choice Coverage (ECC):**
  * Requires that every block from every partition is used in at least one test case ($\max(\vert{}P_i\vert{}) = 3$ tests minimum). TC03 covers the evolution model partition $(b_5)$ paired with deep collections $(b_8)$, while TC04 ensures the hyphenated slug block $(b_3)$ and item model $(b_5)$ are exercised, achieving 100% block coverage across all defined partitions.
* **Pair-Wise Coverage (PWC):**
  * Ensures that every combination of blocks from any two partitions occurs in at least one test case. TC05 tests the pair $(b_1, b_4)$ interacting with collection size verification $(b_8)$. TC06 exercises the pairwise interaction between numeric identification and regional world models $(b_1, b_6)$ interacting with stream URL filtering $(b_8)$.
* **Base Choice Coverage (BCC):**
  * A primary "Base Choice" is chosen representing the standard operational state: querying a Pokemon by numeric ID and verifying deep nested arrays $(b_1, b_4, b_8)$ in TC07 (Snorlax HP stat at index 0). In TC08, following the formal rule of varying one block while holding all other base choices constant, the target stat evaluation is varied to index 1 (Eevee Attack stat) while maintaining identical partition membership $(b_1, b_4, b_8)$.
* **Multiple Base Choice Coverage (MBCC):**
  * Introduces an additional base choice to test alternative operational flows. Base Choice 2 is established in TC09 as $(b_1, b_4, b_7)$, testing Mewtwo with focus on overall entity attributes. In TC10, the identifier format is varied from integer ID ($b_1$) to a multi-word hyphenated slug ($b_3$) while holding the verification scope constant ($b_7$), testing `"lavender-town"` under `Location` model.
